function robot = GA_panda()
% The model of the robot
robot.n = 7;
robot.g = [0; 0; -9.81];
robot.urdf = "panda.urdf";
% robot.g = [0; 0; 0];

% The motor between adjacent joint frames generated by MDH parameter
robot.Mj(:, 1) = GA_DHmotor([      0;     0; 0.333; 0], 1);
robot.Mj(:, 2) = GA_DHmotor([      0; -pi/2;     0; 0], 1);
robot.Mj(:, 3) = GA_DHmotor([      0;  pi/2; 0.316; 0], 1);
robot.Mj(:, 4) = GA_DHmotor([ 0.0825;  pi/2;     0; 0], 1);
robot.Mj(:, 5) = GA_DHmotor([-0.0825; -pi/2; 0.384; 0], 1);
robot.Mj(:, 6) = GA_DHmotor([      0;  pi/2;     0; 0], 1);
robot.Mj(:, 7) = GA_DHmotor([  0.088;  pi/2;     0; 0], 1);
robot.Mj(:, 8) = GA_DHmotor([      0;     0; 0.107; 0], 1); % end effector is frame {i+1}
...
% The init motor
% robot.Mc(:, 1) = GA_prodM(GA_motor([rc(:, 1); 0; 0; 0], 1, 2), robot.Mj(:, 1));
% for i = 2:robot.n
%     robot.Mc(:, i) = GA_prodM(GA_prodM(GA_motor([rc(:, i); 0; 0; 0], 1, 2), robot.Mj(:, i)), GA_motor([-rc(:, i-1); 0; 0; 0], 1, 2)); % from the mass center frame {i-1} to the mass center frame {i}
% end
% robot.Mc(:, robot.n+1) = GA_prodM(robot.Mj(:, robot.n+1), GA_motor([-rc(:, robot.n); 0; 0; 0], 1, 2));

% The initial position of the joint axis
for i = 1:robot.n
    % robot.Lc(:, i) = PGA_Frame2([0; 0; 0; 0; 0; 1], GA_motor([-rc(:, i); 0; 0; 0], 1, 2)); % in the mass center frame
