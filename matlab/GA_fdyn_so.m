function [p2ddq_pqpq, p2ddq_pdqpq, p2ddq_pdqpdq, p2ddq_ptaupq] = GA_fdyn_so(rbt, q, dq, tau, fext)
if nargin == 4
    fext = zeros(6, 1);
end
ddq = GA_fdyn(rbt, q, dq, tau);
...
% FD-SO
p2tau_pdqpq = permute(p2tau_pqpdq, [1, 3, 2]);
p2tau_pddqpq = permute(p2tau_pqpddq, [1, 3, 2]);
tmp1 = pagemtimes(p2tau_pddqpq, pddq_pq);
tmp2 = pagemtimes(p2tau_pddqpq, pddq_pdq);

p2ddq_pqpq = -pagemtimes(pddq_ptau, p2tau_pqpq + tmp1 + permute(tmp1, [1, 3, 2]));
p2ddq_pdqpq = -pagemtimes(pddq_ptau, p2tau_pdqpq + tmp2);
p2ddq_pdqpdq = -pagemtimes(pddq_ptau, p2tau_pdqpdq);
p2ddq_ptaupq = -pagemtimes(pagemtimes(pddq_ptau, p2tau_pddqpq), pddq_ptau);
end